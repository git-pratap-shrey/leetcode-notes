---
title: "Minimum Lights to Illuminate a Road"
slug: minimum-lights-to-illuminate-a-road
date: "2026-07-03"
...
2. **Boundary Logic**: In the line `int end = Math.min(i+2, n-1);`, `end` is calculated but only used to index `diff[end+1]`. This could be simplified to `diff[Math.min(i + 3, n + 1)]--` to reduce variable overhead, though the current way is more readable.
...
## Revision Report: Minimum Lights to Illuminate a Road
