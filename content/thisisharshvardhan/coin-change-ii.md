...
- **Improvements**: 
    ...
    - The `if (coins[ind] <= amount)` block can be simplified: the logic for `ans2` (skipping the coin) is identical in both the `if` and `else` branches and can be moved outside the conditional.
