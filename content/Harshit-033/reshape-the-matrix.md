--- title: "Reshape the Matrix" slug: reshape-the-matrix date: "2026-06-04" ---  # My Solution ~~~class Solution {
public:
...
- **Improvements**:
    - The logic `ans[row][col] = mat[i][j]` can be written using a single loop from $0$ to $(m \times n) - 1$ by calculating indices: `ans[k / c][k % c] = mat[k / n][k % n]`. However, the current nested loop approach is slightly more performant as it avoids repeated division/modulo operations.  ---  # Question Revision ### Reshape the Matrix
