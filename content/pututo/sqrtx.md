...
- **Improvements**: 
    ...
    - The range could be narrowed to $r = \min(x, 46340)$ if using `int` to avoid `long long` entirely (since $\lfloor\sqrt{2^{31}-1}\rfloor = 46340$), but the current `long long` approach is cleaner.
